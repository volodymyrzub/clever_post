...
@Path(com.mobile.cleverpost.rest.constants.Path.PARAMETER)
public class ParameterResource {
	...
	public Parameter get(@PathParam("id") long id) {
		Parameter item = null;
		try {
			item = DAOManager.get(Parameter.class, id);
		} catch (Exception e) {
	...
	public List<Parameter> getList() {
		...
		try {
			list = DAOManager.load(Parameter.class);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public String getCount() {
		int count = -1;
		try {
			count = DAOManager.load(Parameter.class).size();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void create(JAXBElement<Parameter> item) {
		try {
